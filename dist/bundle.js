...
(function (global) {
	...
	global.util.onDomReady().then(function () {
		...
		global.ReactDOM.render(React.createElement(
			Router,
			...
			React.createElement(
				...
				React.createElement(Route, { path: "/character/:id", component: Character })
			)
	...
	global.Character = React.createClass({
		...
		render: function () {
			...
			if (this.state.charData.id) {
				return React.createElement(
					'div',
					{ className: 'characterDetail', style: { backgroundImage: 'url(' + charData.thumbnail.path + '.' + charData.thumbnail.extension + ')' } },
					React.createElement(
						'h2',
						null,
						charData.name
			...
			return React.createElement(
				...
				React.createElement(
					...
					self.state.characters.map(function (character) {
						return React.createElement(IndexCharacter, { key: character.id, id: character.id, name: character.name, thumbnail: character.thumbnail });
					})
				)
			);
		}
	...
	global.IndexCharacter = React.createClass({
		...
		render: function () {
			...
			return React.createElement(
				...
				React.createElement(
					Link,
					{ to: '/character/' + self.props.id, style: { backgroundImage: 'url(' + self.props.thumbnail.path + '.' + self.props.thumbnail.extension + ')' } },
					React.createElement(
						'span',
						{ className: 'caption' },
						self.props.name
